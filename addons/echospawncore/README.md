...
Provides `spawn.difficulty_scaling`, `spawn.faction_rules`, `spawn.hazard_rules`, `spawn.poi_rules`, `spawn.profiles`, `spawn.weather_rules` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echospawncore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `spawn_core` |
...
Required modules: `echoadaptercore`, `echocontentcore`, `echocore`, `echoplatformcore`, `echoprogressioncore`, `echoschemacore`, `echosocialcore`, `echovalidationcore`

Optional modules: None declared.

Provides: `spawn.difficulty_scaling`, `spawn.faction_rules`, `spawn.hazard_rules`, `spawn.poi_rules`, `spawn.profiles`, `spawn.weather_rules`

Consumes: `content.references`, `echo.core`, `platform.contracts`, `schema.registry`, `social.factions`, `validation.pack`
...
| File | Requirement |
| --- | --- |
| `echospawncore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echospawncore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echospawncore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echospawncore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echospawncore
```

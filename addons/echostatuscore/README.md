...
Provides `status.effects`, `status.exposure`, `status.resistance` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echostatuscore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `status_core` |
...
Provides: `status.effects`, `status.exposure`, `status.resistance`
...
```sh
node scripts/generate-module-release.mjs --module echostatuscore
```

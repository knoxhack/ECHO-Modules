...
Provides `ai.tasks`, `ai.task_queue`, `ai.prompt_bundle`, `ai.next_phase_prompt`, `ai.run_report`, `ai.safe_commands` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echoagentcore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `ai_agent_core` |
...
Consumes: `echo.core`, `platform.contracts`, `schema.registry`, `validation.pack`, `runtime.health`
...
| File | Requirement |
| --- | --- |
| `echoagentcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echoagentcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echoagentcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echoagentcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echoagentcore
```

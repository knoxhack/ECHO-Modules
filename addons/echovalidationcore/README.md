...
Provides `validation.pack` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echovalidationcore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `platform` |
...
Required modules: `echocore`, `echoplatformcore`, `echoschemacore`

Optional modules: None declared.

Provides: `validation.pack`

Consumes: `echo.core`, `platform.contracts`, `schema.registry`
...
| File | Requirement |
| --- | --- |
| `echovalidationcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echovalidationcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echovalidationcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echovalidationcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echovalidationcore
```

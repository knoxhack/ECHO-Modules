...
/**
 * Bridges {echometadatacore} spine events into the AdapterCore truth layer.
 */
public final class EchoMetadatacoreTruthBridge {
    private static final String SOURCE_MODULE = "echometadatacore";
    private static boolean registered;

    private EchoMetadatacoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echometadatacore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echometadatacore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

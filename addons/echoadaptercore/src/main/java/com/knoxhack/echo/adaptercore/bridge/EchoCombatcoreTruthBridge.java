...
/**
 * Bridges {echocombatcore} spine events into the AdapterCore truth layer.
 */
public final class EchoCombatcoreTruthBridge {
    private static final String SOURCE_MODULE = "echocombatcore";
    private static boolean registered;

    private EchoCombatcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echocombatcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echocombatcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

...
/**
 * Bridges {echoarmory} spine events into the AdapterCore truth layer.
 */
public final class EchoArmoryTruthBridge {
    private static final String SOURCE_MODULE = "echoarmory";
    private static boolean registered;

    private EchoArmoryTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoarmory".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echoarmory:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

...
/**
 * Bridges {echovehiclecore} spine events into the AdapterCore truth layer.
 */
public final class EchoVehiclecoreTruthBridge {
    private static final String SOURCE_MODULE = "echovehiclecore";
    private static boolean registered;

    private EchoVehiclecoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echovehiclecore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echovehiclecore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

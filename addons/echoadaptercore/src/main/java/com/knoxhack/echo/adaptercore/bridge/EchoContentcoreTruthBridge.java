...
/**
 * Bridges {echocontentcore} spine events into the AdapterCore truth layer.
 */
public final class EchoContentcoreTruthBridge {
    private static final String SOURCE_MODULE = "echocontentcore";
    private static boolean registered;

    private EchoContentcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echocontentcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echocontentcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

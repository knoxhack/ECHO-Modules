...
/**
 * Bridges {@code echoindex} recipe/catalog events into the AdapterCore truth layer.
 */
public final class EchoIndexTruthBridge {
    private static final String SOURCE_MODULE = "echoindex";
    private static boolean registered;

    private EchoIndexTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoindex".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "index:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

...
/**
 * Bridges {echofamiliarcore} spine events into the AdapterCore truth layer.
 */
public final class EchoFamiliarcoreTruthBridge {
    private static final String SOURCE_MODULE = "echofamiliarcore";
    private static boolean registered;

    private EchoFamiliarcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echofamiliarcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echofamiliarcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

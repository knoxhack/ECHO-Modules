...
/**
 * Bridges {@code echosignalos} terminal/mission events into the AdapterCore truth layer.
 */
public final class EchoSignalOSTruthBridge {
    private static final String SOURCE_MODULE = "echosignalos";
    private static boolean registered;

    private EchoSignalOSTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echosignalos".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "signalos:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

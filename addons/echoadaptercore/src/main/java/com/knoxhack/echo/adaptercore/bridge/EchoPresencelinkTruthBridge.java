...
/**
 * Bridges {echopresencelink} spine events into the AdapterCore truth layer.
 */
public final class EchoPresencelinkTruthBridge {
    private static final String SOURCE_MODULE = "echopresencelink";
    private static boolean registered;

    private EchoPresencelinkTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echopresencelink".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echopresencelink:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

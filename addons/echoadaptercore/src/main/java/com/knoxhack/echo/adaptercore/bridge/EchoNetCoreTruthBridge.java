...
/**
 * Bridges {@code echonetcore} packet routing events into the AdapterCore truth layer.
 */
public final class EchoNetCoreTruthBridge {
    private static final String SOURCE_MODULE = "echonetcore";
    private static boolean registered;

    private EchoNetCoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echonetcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "netcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

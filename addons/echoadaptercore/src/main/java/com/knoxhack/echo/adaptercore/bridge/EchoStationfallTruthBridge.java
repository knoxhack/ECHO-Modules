...
/**
 * Bridges {echostationfall} spine events into the AdapterCore truth layer.
 */
public final class EchoStationfallTruthBridge {
    private static final String SOURCE_MODULE = "echostationfall";
    private static boolean registered;

    private EchoStationfallTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echostationfall".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echostationfall:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

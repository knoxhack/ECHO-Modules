...
/**
 * Bridges {@code echolens} scan/discovery events into the AdapterCore truth layer.
 */
public final class EchoLensTruthBridge {
    private static final String SOURCE_MODULE = "echolens";
    private static boolean registered;

    private EchoLensTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echolens".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "lens:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

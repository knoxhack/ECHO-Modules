...
/**
 * Bridges {echocreaturecore} spine events into the AdapterCore truth layer.
 */
public final class EchoCreaturecoreTruthBridge {
    private static final String SOURCE_MODULE = "echocreaturecore";
    private static boolean registered;

    private EchoCreaturecoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echocreaturecore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echocreaturecore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

...
/**
 * Bridges {echohudcore} spine events into the AdapterCore truth layer.
 */
public final class EchoHudcoreTruthBridge {
    private static final String SOURCE_MODULE = "echohudcore";
    private static boolean registered;

    private EchoHudcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echohudcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echohudcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

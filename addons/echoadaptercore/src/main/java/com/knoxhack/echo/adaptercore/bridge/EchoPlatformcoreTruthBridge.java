...
/**
 * Bridges {echoplatformcore} spine events into the AdapterCore truth layer.
 */
public final class EchoPlatformcoreTruthBridge {
    private static final String SOURCE_MODULE = "echoplatformcore";
    private static boolean registered;

    private EchoPlatformcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoplatformcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echoplatformcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

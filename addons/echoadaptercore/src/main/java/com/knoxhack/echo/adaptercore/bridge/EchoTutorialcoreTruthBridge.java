...
/**
 * Bridges {echotutorialcore} spine events into the AdapterCore truth layer.
 */
public final class EchoTutorialcoreTruthBridge {
    private static final String SOURCE_MODULE = "echotutorialcore";
    private static boolean registered;

    private EchoTutorialcoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echotutorialcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echotutorialcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

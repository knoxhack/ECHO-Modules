...
/**
 * Bridges {@code echoholomap} marker/route events into the AdapterCore truth layer.
 */
public final class EchoHoloMapTruthBridge {
    private static final String SOURCE_MODULE = "echoholomap";
    private static boolean registered;

    private EchoHoloMapTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoholomap".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "holomap:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

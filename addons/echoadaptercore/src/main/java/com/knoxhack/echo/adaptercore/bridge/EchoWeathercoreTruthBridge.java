...
/**
 * Bridges {echoweathercore} spine events into the AdapterCore truth layer.
 */
public final class EchoWeathercoreTruthBridge {
    private static final String SOURCE_MODULE = "echoweathercore";
    private static boolean registered;

    private EchoWeathercoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoweathercore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echoweathercore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

...
/**
 * Bridges {echocore} spine events into the AdapterCore truth layer.
 */
public final class EchoCoreTruthBridge {
    private static final String SOURCE_MODULE = "echocore";
    private static boolean registered;

    private EchoCoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echocore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echocore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

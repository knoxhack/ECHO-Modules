...
/**
 * Bridges {@code echoworldcore} region enter/exit and hazard events into the AdapterCore truth layer.
 */
public final class EchoWorldCoreTruthBridge {
    private static final String SOURCE_MODULE = "echoworldcore";
    private static boolean registered;

    private EchoWorldCoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoworldcore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "worldcore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

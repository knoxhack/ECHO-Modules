...
/**
 * Bridges {echowiki} spine events into the AdapterCore truth layer.
 */
public final class EchoWikiTruthBridge {
    private static final String SOURCE_MODULE = "echowiki";
    private static boolean registered;

    private EchoWikiTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echowiki".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echowiki:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

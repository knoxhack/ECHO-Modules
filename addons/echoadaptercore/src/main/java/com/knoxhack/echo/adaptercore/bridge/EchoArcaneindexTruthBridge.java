...
/**
 * Bridges {echoarcaneindex} spine events into the AdapterCore truth layer.
 */
public final class EchoArcaneindexTruthBridge {
    private static final String SOURCE_MODULE = "echoarcaneindex";
    private static boolean registered;

    private EchoArcaneindexTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoarcaneindex".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echoarcaneindex:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

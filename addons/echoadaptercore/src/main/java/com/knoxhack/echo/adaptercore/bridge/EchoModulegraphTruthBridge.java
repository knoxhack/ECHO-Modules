...
/**
 * Bridges {echomodulegraph} spine events into the AdapterCore truth layer.
 */
public final class EchoModulegraphTruthBridge {
    private static final String SOURCE_MODULE = "echomodulegraph";
    private static boolean registered;

    private EchoModulegraphTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echomodulegraph".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echomodulegraph:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

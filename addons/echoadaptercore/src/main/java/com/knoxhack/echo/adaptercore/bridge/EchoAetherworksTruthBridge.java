...
/**
 * Bridges {echoaetherworks} spine events into the AdapterCore truth layer.
 */
public final class EchoAetherworksTruthBridge {
    private static final String SOURCE_MODULE = "echoaetherworks";
    private static boolean registered;

    private EchoAetherworksTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echoaetherworks".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echoaetherworks:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

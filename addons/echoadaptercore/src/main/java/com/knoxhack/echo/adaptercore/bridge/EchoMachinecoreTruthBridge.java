...
/**
 * Bridges {echomachinecore} spine events into the AdapterCore truth layer.
 */
public final class EchoMachinecoreTruthBridge {
    private static final String SOURCE_MODULE = "echomachinecore";
    private static boolean registered;

    private EchoMachinecoreTruthBridge() {
    }
            ...
            busClass.getMethod("register", java.util.function.Consumer.class)
                    .invoke(null, (java.util.function.Consumer<Object>) event -> {
                        ...
                        try {
                            ...
                            if (!"echomachinecore".equals(sourceModule)) {
                                return;
                            }
                            ...
                            EchoAdapterCoreSpinePublisher.instance().publish(
                                    ...
                                    "echomachinecore:" + canonicalEvent + ":" + (playerId.isBlank() ? "world" : playerId) + ":" + System.currentTimeMillis());

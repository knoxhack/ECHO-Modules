...
Provides `pack.profile`, `pack.repair` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echopackcore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `packos` |
...
Required modules: `echoadaptercore`, `echocore`, `echoplatformcore`, `echovalidationcore`

Optional modules: None declared.

Provides: `pack.profile`, `pack.repair`

Consumes: `echo.core`, `platform.contracts`, `validation.pack`
...
| File | Requirement |
| --- | --- |
| `echopackcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echopackcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echopackcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echopackcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echopackcore
```

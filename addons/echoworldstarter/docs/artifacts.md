# ECHO World Starter Artifact Notes

This file documents the release outputs expected for `echoworldstarter` version
`0.1.0`.
...
| File | Requirement |
| --- | --- |
| `echoworldstarter-0.1.0-neoforge.jar` | Required for ECHO NeoForge editions. |
| `echoworldstarter-0.1.0.echo-addon` | Required for ECHO Native editions. |
| `echoworldstarter-0.1.0-standalone.jar` | Required for ECHO Standalone editions. |
| `echoworldstarter-0.1.0-sources.jar` | Always required for traceability and developer debugging. |
| `echoworldstarter-0.1.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |
...
- `META-INF/echo.mod.json` is the cross-runtime descriptor and must match the released module ID, version, dependency contracts, and foundation starter namespaces.
...
Launcher modpack updates treat ECHO World Starter as an individual managed module

# ECHO: AccessibilityCore Artifact Notes

This file documents the release outputs expected for `echoaccessibilitycore` version `0.1.0`.

## Artifact Matrix

| File | Requirement |
| --- | --- |
| `echoaccessibilitycore-0.1.0-neoforge.jar` | Required for NeoForge editions when this module is selected. |
| `echoaccessibilitycore-0.1.0.echo-addon` | Required for ECHO Native editions when this module is selected. |
| `echoaccessibilitycore-0.1.0-standalone.jar` | Required for Standalone editions when this module is selected. |
| `echoaccessibilitycore-0.1.0-sources.jar` | Always required for traceability and developer debugging. |
| `echoaccessibilitycore-0.1.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |

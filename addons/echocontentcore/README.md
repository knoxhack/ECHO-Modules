...
Required modules: `echoadaptercore`, `echocore`, `echoplatformcore`, `echoschemacore`, `echovalidationcore`

Optional modules: None declared.

Provides: `content.availability`, `content.gates`, `content.ownership`, `content.references`

Consumes: `echo.core`, `platform.contracts`, `schema.registry`, `validation.pack`
...
| File | Requirement |
| --- | --- |
| `echocontentcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echocontentcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echocontentcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echocontentcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echocontentcore
```

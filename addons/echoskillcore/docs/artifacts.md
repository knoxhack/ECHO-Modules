# ECHO: SkillCore Artifact Notes

This file documents the release outputs expected for `echoskillcore` version `0.1.0`.

## Artifact Matrix

| File | Requirement |
| --- | --- |
| `echoskillcore-0.1.0-neoforge.jar` | Required for NeoForge editions when this module is selected. |
| `echoskillcore-0.1.0.echo-addon` | Required for ECHO Native editions when this module is selected. |
| `echoskillcore-0.1.0-standalone.jar` | Required for Standalone editions when this module is selected. |
| `echoskillcore-0.1.0-sources.jar` | Always required for traceability and developer debugging. |
| `echoskillcore-0.1.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |

...
Provides `theme.asset_kits`, `theme.block_palettes`, `theme.echo_platform`, `theme.cyberglass`, `theme.render_profiles`, `theme.tokens`, `theme.ui_skins` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echothemecore` |
| Version | `1.3.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `theme_core` |
...
| File | Requirement |
| --- | --- |
| `echothemecore-1.3.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echothemecore-1.3.0.echo-addon` | Required for Ashfall Native Edition. |
| `echothemecore-1.3.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echothemecore-1.3.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echothemecore
```

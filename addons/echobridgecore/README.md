...
Provides `bridge.codex`, `bridge.cyberdex`, `bridge.streaming_logs`, `bridge.safe_actions`, `bridge.confirmations`, `bridge.heartbeat` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echobridgecore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `bridge_core` |
...
Consumes: `platform.contracts`, `runtime.health`, `ai.prompt_bundle`
...
| File | Requirement |
| --- | --- |
| `echobridgecore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echobridgecore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echobridgecore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echobridgecore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echobridgecore
```

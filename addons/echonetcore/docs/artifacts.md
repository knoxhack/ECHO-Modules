# ECHO: NetCore Artifact Notes

This file documents the release outputs expected for `echonetcore` version `1.0.0`.

## Artifact Matrix

| File | Requirement |
| --- | --- |
| `echonetcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echonetcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echonetcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echonetcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
| `echonetcore-1.0.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |

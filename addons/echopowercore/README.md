...
Provides `power.diagnostics`, `power.networks`, `power.storage`, `power.transfer` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echopowercore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `power_contracts` |
| Side | `common` |
| Trust | `official` |
...
Provides: `power.diagnostics`, `power.networks`, `power.storage`, `power.transfer`
...
| File | Requirement |
| --- | --- |
| `echopowercore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echopowercore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echopowercore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echopowercore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echopowercore
```

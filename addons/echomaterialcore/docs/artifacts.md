# ECHO Material Core Artifact Notes

This file documents the release outputs expected for `echomaterialcore` version
`0.1.0`.
...
| File | Requirement |
| --- | --- |
| `echomaterialcore-0.1.0-neoforge.jar` | Required for ECHO NeoForge editions. |
| `echomaterialcore-0.1.0.echo-addon` | Required for ECHO Native editions. |
| `echomaterialcore-0.1.0-standalone.jar` | Required for ECHO Standalone editions. |
| `echomaterialcore-0.1.0-sources.jar` | Always required for traceability and developer debugging. |
| `echomaterialcore-0.1.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |
...
- `META-INF/echo.mod.json` is the cross-runtime descriptor and must match the released module ID, version, dependency contracts, and foundation material namespaces.
...
Launcher modpack updates treat ECHO Material Core as an individual managed module

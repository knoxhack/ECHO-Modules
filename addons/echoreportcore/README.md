...
Provides `reports.contracts`, `commandcenter.reports`, `launcher.status`, `support_bundles.manifest`, `release.readiness` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echoreportcore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `report_core` |
...
Required modules: `echocore`, `echoplatformcore`, `echoadaptercore`, `echoschemacore`, `echovalidationcore`, `echopackcore`, `echomodulegraph`, `echohealthcore`, `echoagentcore`, `echoassetcore`

Optional modules: None declared.

Provides: `reports.contracts`, `commandcenter.reports`, `launcher.status`, `support_bundles.manifest`, `release.readiness`

Consumes: `echo.core`, `platform.contracts`, `adapter.neoforge`, `schema.registry`, `validation.pack`, `pack.profile`, `pack.repair`, `module.graph`, `feature.graph`, `runtime.health`, `ai.task_queue`
...
| File | Requirement |
| --- | --- |
| `echoreportcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echoreportcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echoreportcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echoreportcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echoreportcore
```

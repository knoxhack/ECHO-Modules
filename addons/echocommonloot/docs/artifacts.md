# ECHO Common Loot Artifact Notes

This file documents the release outputs expected for `echocommonloot` version
`0.1.0`.
...
| File | Requirement |
| --- | --- |
| `echocommonloot-0.1.0-neoforge.jar` | Required for ECHO NeoForge editions. |
...
- `META-INF/echo.mod.json` is the cross-runtime descriptor and must match the released module ID, version, dependency contracts, and foundation loot namespaces.
...
Launcher modpack updates treat ECHO Common Loot as an individual managed module

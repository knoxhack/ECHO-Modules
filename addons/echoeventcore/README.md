...
Provides `event.scheduler`, `event.validation`, `event.world_events` for the ECHO module graph.

## Module Identity

| Field | Value |
| --- | --- |
| Module ID | `echoeventcore` |
| Version | `1.0.0` |
| Type | `addon` |
| Kind | `library` |
| Role | `event_core` |
...
Provides: `event.scheduler`, `event.validation`, `event.world_events`
...
| File | Requirement |
| --- | --- |
| `echoeventcore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echoeventcore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echoeventcore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echoeventcore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
...
```sh
node scripts/generate-module-release.mjs --module echoeventcore
```

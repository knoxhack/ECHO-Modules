# ECHO: VehicleCore Artifact Notes

This file documents the release outputs expected for `echovehiclecore` version `1.0.0`.

## Artifact Matrix

| File | Requirement |
| --- | --- |
| `echovehiclecore-1.0.0-neoforge.jar` | Required for Ashfall NeoForge Edition. |
| `echovehiclecore-1.0.0.echo-addon` | Required for Ashfall Native Edition. |
| `echovehiclecore-1.0.0-standalone.jar` | Required for Ashfall Standalone Edition. |
| `echovehiclecore-1.0.0-sources.jar` | Always required for traceability and developer debugging. |
| `echovehiclecore-1.0.0-content-graph.json` | Required; Release-Index catalogable sidecar containing the canonical `.ECHO Content Graph`. |
